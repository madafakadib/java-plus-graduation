package ru.practicum.compilation_service.repository;
...
public class CompilationRepositoryImpl implements CompilationRepository {
    ...
    public Compilation postCompilation(Compilation compilation) {
        ...
        if (compilation.getEventId() != null && !compilation.getEventId().isEmpty()) {
            saveEventsRelations(compId, compilation.getEventId());
        }
    ...
    public List<Compilation> getCompilations(Boolean pinned, int from, int size) {
        ...
        Map<Long, List<Long>> eventsByCompId = new HashMap<>();
        ...
        for (Compilation comp : compilations) {
            comp.setEventId(eventsByCompId.getOrDefault(comp.getId(), Collections.emptyList()));
        }
        return compilations;
    }
    ...
    private void saveEventsRelations(Long compId, List<Long> eventsId) {
        String sql = "INSERT INTO compilation_events (compilation_id, event_id) VALUES (?, ?)";

        jdbcTemplate.batchUpdate(sql, eventsId, eventsId.size(), (ps, eventId) -> {
            ps.setLong(1, compId);
