package ru.practicum.comment_service.repository;
...
public interface CommentRepository extends JpaRepository<Comment, Long> {
    ...
    @Query("SELECT c.eventId, COUNT(c) " +
            "FROM Comment c " +
            "WHERE c.eventId IN :eventIds AND c.status = :status " +
            "GROUP BY c.eventId")
    List<CommentCountDto> countByEventIdInAndStatus(@Param("eventIds") List<Long> eventIds, @Param("status") CommentStatus status);
